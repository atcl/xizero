...
namespace game
{
	...
	/*OK*/ pure inline bool inside(sint x,sint y,sint a,sint b,sint c,sint d);
	                   bool collision(const fvector& x,fixed r,const fvector& a);
...
bool game::collision(const fvector& x,fixed r,const fvector& a) //include level bounds check
{
	const fvector t = x-a;
	return fx::sqr(fx::mul(t.x,t.x)+fx::mul(t.y,t.y))<r;
}
