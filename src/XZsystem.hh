...
#include <cstdio> //putchar,getchar,fopen,fseek,ftell,fread,fclose
...
namespace system
{
	...
	/*OK*/        void  slp(long x) { long e=clock()+x*CLOCKS_PER_SEC/1000; while(clock()<e) { ; } }
	/*OK*/        void  say(const char* x,bool y) { long i=0; while(x[i]!=0) { putchar(x[i]); i++; } if(y) { putchar('\n'); } }
	/*OK*/        char* ldf(const char* x) { FILE *f; f=fopen(x,"rb"); fseek(f,0,SEEK_END); const long n=ftell(f); char* r=new char[n]; fseek(f,0,SEEK_SET); fread(r,n,1,f); fclose(f); return r; } 
